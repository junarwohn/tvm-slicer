...
if args.build == 1:
    lib.export_library("UNet_M[{}-{}-{}-{}]_Q[{}]_full_jetson.so".format(*model_config, quantization_level))
    param_bytes = tvm.runtime.save_param_dict(lib.get_params())
    with open("UNet_M[{}-{}-{}-{}]_Q[{}]_full_jetson.params".format(*model_config, quantization_level), "wb") as f:
        f.write(param_bytes)
