...
combi = combination(partition_points[1:-2])
combi = [c + [partition_points[-2]] for c in combi]

for com in combi:
    print("python3 slicing_graph.py -m {} -o {} -i {} -t {} -p".format(args.model, args.opt_level, args.img_size, args.target), *([first_index] + com), partition_points[-1])
