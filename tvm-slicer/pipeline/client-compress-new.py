...
parser.add_argument('--reduce', '-r', type=int, default=1, help='reduce')
...
def generate_img(q):
    ...
    while (cap.isOpened()):
        ...
        for out in outs:
            ...
            msg_body += out_byte
        q.put(frame)
        reduced_msg = msg_body[:int(len(msg_body) / args.reduce)]
        total_send_msg_size = len(reduced_msg) 
        send_msg = struct.pack('i', total_send_msg_size) + reduced_msg
        # Send object
...
def recv_img(q):
    ...
    while True:
        ...
        # out = np.frombuffer(recv_msg[:total_recv_msg_size], np.float32).reshape(tuple(final_output_shape))
        img_in_rgb = q.get()
