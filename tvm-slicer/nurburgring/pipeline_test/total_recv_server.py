...
print(model_info['heads'][-1][0])
print(model_info["attrs"]["shape"][1][model_info['heads'][-1][0]])
output_shapes += np.array(model_info["attrs"]["shape"][1][model_info['heads'][-1][0]]).tobytes()
...
while True:
    ...
    out = back_model.get_output(4).asnumpy().astype(np.float32)
