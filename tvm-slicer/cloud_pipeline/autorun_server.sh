...
while getopts ":r:f:i:m:t:o:a:n:" opt; 
do
	case $opt in
		r) ratio="$OPTARG"
			;;
...
#for i in ${nodes[@]:0:1}
do
	cmd="python3 server-resnet.py --ip=${ip_address} --partition_point=${i} --img_size=${img_size} --model=${model} --target=${target} --opt_level=${opt_level} --ntp_enable=${ntp_enable}"
	echo $cmd
